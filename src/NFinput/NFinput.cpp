...
component::component(shared_ptr<TemplateMolecule> t, string name)
{
	mt=0;
	uniqueId="";
	symPermutationName="";
	...
	try {
		...
		for ( pRxnRule = pListOfReactionRules->FirstChildElement("ReactionRule"); pRxnRule != 0; pRxnRule = pRxnRule->NextSiblingElement("ReactionRule"))
		{
			...
			for( unsigned int p=0; p<permutations.size(); p++)
			{
				...
				// ASS2019 WE NEED TO TACKLE COMPARTMENT CHANGES
				// Example operation XML:
                                //<ListOfOperations>
				...
				{
					//Make sure all the information about the compartment change is here
                                        string id, source, destination;
                                        if(!pChangeCompartment->Attribute("source") || !pChangeCompartment->Attribute("destination") || !pChangeCompartment->Attribute("id")) {
					...
					// TODO: Do we need to deal w/ symmetry here?
					// TODO: Use ID to find what needs to be modified
		...
		int &suggestedTraversalLimit)
{
	try {
		...
		for ( pMol = pListOfMol->FirstChildElement("Molecule"); pMol != 0; pMol = pMol->NextSiblingElement("Molecule"))
		{
			...
			moltype->addTemplateMolecule(tempmol);
			if(verbose) cout<<"\t\t\t\tIncluding Molecule of type: "<<molName<<" with local id: " << molUid<<endl;
